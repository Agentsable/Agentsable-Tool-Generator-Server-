...
* **`signature`**: JSON Schema defining `inputs`, `outputs`, and critically, **`errors`**. Explicit error schemas allow AI Agents to read `actionable_advice` and self-correct when they make a mistake.
...
* **`tests`**: An embedded array of HTTP-driven test definitions (`name`, `payload`, `expect`) to validate the tool's logic and error handling locally. Additional external tests can also be placed in a sibling `[tool_name]_tests.json` file.
...
* **`origin`**: Either `"agent"`, `"sdk_rest"`, or `"mcp_client"`. (Useful for analytics, logging, or strict feature-flagging, though the output format remains identical).
...
3. **Explicit Tool Dependencies:** You must never call `internalFetch()` blindly. If a tool invokes a sibling tool, that sibling's name MUST be declared in the `tool_dependencies` array in the config, otherwise the Deno router will block it.
